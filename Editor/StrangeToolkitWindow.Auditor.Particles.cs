    ...
    public partial class StrangeToolkitWindow
    {
        private void ScanParticles()
        {
            ...
            foreach (var ps in particles)
            {
                ...
                if (ps.collision.enabled)
                    _particleIssues.Add(new ParticleIssue { sys = ps, reason = "Collision Enabled (Expensive)" });
            }
        }
        ...
        private void DrawParticleAuditor()
        {
            if (_particleIssues.Count > 0)
            {
                ...
                if (GUILayout.Button("Reduce Count by 50% (All Listed)"))
                {
                    ReduceParticleCounts(_particleIssues.Select(x => x.sys).ToList(), 0.5f);
                }
